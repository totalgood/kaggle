...
ds = util.dataset_from_dataframe(df_freq, normalize=False, delays=[0], inputs=feature_labels, outputs=class_labels,
                                 verbosity=1)
nn = util.ann_from_ds(ds, N_hidden=[12, 9] verbosity=1)
trainer = util.build_trainer(nn, ds=ds, verbosity=1)
trainer.trainUntilConvergence(maxEpochs=300, verbose=True)
...
NetworkWriter.writeToFile(trainer.module, 'NN-linear-regressor-16hidden.xml')

columns = feature_labels + target_labels + ['Predicted--{}'.format(outp) for outp in target_labels]
table = [list(i) + list(t) + list(trainer.module.activate(i)) for i, t in
         zip(trainer.ds['input'], trainer.ds['target'])]
...
def write_submission_file(df, path='submission.csv', class_labels=class_labels):
    df[['id'] + list(class_labels)].to_csv(path)
...
# df_vs = pd.DataFrame.from_csv(DATA_PATH + "test.csv")
...
# vs_inputs = tfidf.fit_transform(df_vs[feature_labels].values).toarray()
# vs_freq = tfidf.transform(vs_inputs).toarray()
# print('Finished transforming the test data using the trained TFIDF.')
